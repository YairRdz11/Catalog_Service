...
namespace CatalogService.BLL.Tests.Classes
{
  public class CategoryServiceTests
  {
    ...
    public async Task CreateAsync_WithValidCategory_ShouldReturnCreatedCategory()
    {
      ...
      _mockCategoryRepository.Setup(r => r.DoesItemExistByNameAsync(categoryDTO.Name))
     .ReturnsAsync(false);

      _mockCategoryRepository.Setup(r => r.CreateAsync(It.IsAny<CategoryDTO>()))
    ...
    public async Task CreateAsync_WithParentCategory_ShouldVerifyParentExists()
    {
      ...
      _mockCategoryRepository.Setup(r => r.GetByIdAsync(parentId))
...
.ReturnsAsync(categoryDTO);
      ...
      _mockProductRepository.Setup(r => r.GetProductsByCategoryAsync(categoryId))
    ...
    public async Task GetListAsync_ShouldReturnAllCategories()
    {
     ...
     new CategoryDTO { Id = Guid.NewGuid(), Name = "Clothing" }
      ...
      _mockCategoryRepository.Setup(r => r.GetListAsync())
  ...
  .ReturnsAsync(existingCategory);
      ...
      _mockCategoryRepository.Setup(r => r.DoesItemExistByNameAsync(updatedCategory.Name))
  .ReturnsAsync(false);

      _mockCategoryRepository.Setup(r => r.UpdateAsync(It.IsAny<CategoryDTO>()))
...
.ReturnsAsync(existingCategory);
  ...
  .ReturnsAsync(updatedCategory);
      ...
      // Act
      // Note: UpdateAsync has a bug - it doesn't await ValidateCategory
      // So the validation runs but doesn't block the execution
      // The method will still complete successfully with invalid data
      var result = await _categoryService.UpdateAsync(updatedCategory);
...
.ReturnsAsync(false);
    ...
    public async Task CreateAsync_WithValidURLFormats_ShouldSucceed(string url)
    {
      ...
      _mockCategoryRepository.Setup(r => r.DoesItemExistByNameAsync(It.IsAny<string>()))
   .ReturnsAsync(false);
